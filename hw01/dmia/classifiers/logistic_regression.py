...
class LogisticRegression:
    ...
    def predict_proba(self, X, append_bias=False):
        ...
        y_proba = np.transpose(np.vstack((1 - X_sig, X_sig)))        
    ...
    def predict(self, X):
        ...
        y_pred = map(lambda x: 1 if x[0] <= x[1] else 0, y_proba)        
    ...
    def loss(self, X_batch, y_batch, reg):
        ...
        X_sig = self.sigmoid(X_batch)
        loss = np.sum(-y_batch * np.log(X_sig) - (1 - y_batch) * np.log(1 - X_sig))                
        dw = np.sum(X_batch * (X_sig - y_batch)[:, np.newaxis], axis=0)                
        
        # Right now the loss is a sum over all training examples, but we want it
        # to be an average instead so we divide by num_train.
        # Note that the same thing must be done with gradient.
        loss /= float(len(y_batch))
        dw /= float(len(y_batch))
        
        # Add regularization to the loss and gradient.
        # Note that you have to exclude bias term in regularization.
        loss += reg * np.linalg.norm(self.w[:-1]) / len(y_batch)        
        dw += float(reg) * np.append(self.w[:-1], 0) / len(y_batch)

        return loss, dw
    
    
    def sigmoid(self, x):        
        return 1.0 / (1 + np.exp(-x.dot(self.w)))
